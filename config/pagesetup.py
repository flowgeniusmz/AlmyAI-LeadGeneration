...
def set_blue_header(varSubtitle):
    st.markdown(f"""<span style="font-weight: bold; color:#0096D7; font-size:2em;">{varSubtitle}</span>""", unsafe_allow_html=True)
...
### 3. PAGE OVERVIEW
def set_page_overview_no_div(varHeader, varText):
        set_blue_header(varHeader)
        st.markdown(f"#### {varText}")
        #st.divider()
